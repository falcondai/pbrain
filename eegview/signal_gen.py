...
def signal_gen(SNR, K2, K3):
    ...
    S1 = np.sin(2*math.pi*freq*t) * float(SNR) * np.std(noise)
    ...
    for k in range(dly2+1, N):
        if (NL == 0):
            ...
            S2[k] = S1[k-dly2]*K2 + np.random.randn(1)[0]
        else:
            np.random.seed(k)
            S2[k] = math.pow(S1[k-dly2], 2) * K2 + np.random.randn(1)[0]

    for k in range(dly3+1, N):
        if (NL == 0):
            #np.random.seed(k)
            S3[k] = S1[k-dly3]*K3 + np.random.randn(1)[0]
        else:
            np.random.seed(k+1)
            S3[k] = math.pow(S1[k-dly3], 2) * K3 + np.random.randn(1)[0]
